...
class TrackServiceTests {
    ...
    inner class Create {
        ...
        fun `creates a new track`() =
            runTest {
                ...
                val track = trackService.create(firstTrackRequest)
            ...
            runTest {
                ...
                val exception =
                    assertThrows<TrackAlreadyExistsException> {
                        trackService.create(firstTrackRequest)
                    }
    ...
    inner class Update {
        ...
        fun `updates an existing track`() =
            runTest {
                whenever(mockTrackRepository.findById(secondTrackId)).thenReturn(secondTrackEntity)
                whenever(mockTrackRepository.save(secondTrackEntity)).thenReturn(secondTrackEntity)

                val updatedTrack = trackService.update(secondTrackId, secondTrackRequest)
                ...
                verify(mockTrackRepository, times(1)).findById(secondTrackId)
        ...
        fun `does not update a track that does not exist`() =
            runTest {
                whenever(mockTrackRepository.findById(secondTrackId)).thenReturn(null)

                val exception =
                    assertThrows<TrackDoesNotExistException> {
                        trackService.update(secondTrackId, secondTrackRequest)
                    }
    ...
    private val secondTrackRequest =
        TrackRequest(
            name = SECOND_TRACK_NAME,
            latitude = SECOND_TRACK_LATITUDE,
            longitude = SECOND_TRACK_LONGITUDE,
